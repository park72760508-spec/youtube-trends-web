...
// 완료 메시지 업데이트
    ...
    filterVideosByTitleKeywords(videos, tier2Keywords = [], tier3Keywords = []) {
      ...
      console.log(`🔍 제목 필터링 시작: ${videos.length}개 영상 중 ${allKeywords.length}개 키워드로 필터링`);
      
      // 영상 객체 또는 비디오 객체를 처리
      const filtered = videos.filter(video => {
        // video 객체 또는 video.video 객체에서 제목 추출
        const title = video.snippet?.title || 
                      video.title || 
                      (video.video && (video.video.snippet?.title || video.video.title)) || 
                      '';
      ...
      console.log(`✅ 제목 필터링 완료: ${videos.length}개 → ${filtered.length}개 (${filtered.length / Math.max(1, videos.length) * 100}%)`);
    ...
    updateLiveCountersUI() {
      ...
      console.log(`🔄 UI 업데이트: 백그라운드 ${totalCollected}개, 검출 ${detected}개, 속도 ${rate}/분`);
        ...
        async runLargeScaleChannelPipeline(keywords, options = {}) {
            const { maxChannels, batchSettings, concurrency = 4, tier2Keywords = [], tier3Keywords = [] } = options;
            ...
            // 🔥 Tier별 키워드 정보 로깅
            if (tier2Keywords.length > 0 || tier3Keywords.length > 0) {
                console.log(`📊 Tier별 필터링 활성화: Tier2(${tier2Keywords.length}개), Tier3(${tier3Keywords.length}개)`);
            }
            ...
            try {
                ...
                for (const keyword of keywords) {
                    ...
                    // 🔥 기존 파이프라인 호출 (배치 크기 제한 + Tier별 키워드 전달)
