...
class OptimizedYoutubeTrendsAnalyzer {
    constructor() {
        ...
        // 최적화된 키워드 (우선순위별) + 사용자 커스텀 병합
        this.optimizedKeywords = {
            tier1: this.loadKeywordTier('hot_custom_tier1', [
                '시니어', '실버', '중년', '노인', '장년', '액티브시니어',
                '황혼', '은퇴', '시니어라이프', '실버세대', '실버케어', '골든에이지'
            ]),
            tier2: this.loadKeywordTier('hot_custom_tier2', [
                '부동산', '주택', '정치', '환경', '기후','범죄', '여행',
                '관광', '건강', '운동', '패션', '소비', '요리', '취미'
            ]),
            tier3: this.loadKeywordTier('hot_custom_tier3', [
                '시니어 운동', '노년 취미', '실버 요리', '중년 건강',
                '시니어 테크', '노인 여행', '실버 댄스', '중년 요리'
            ])
        };
        console.log('🔑 커스텀 키워드 적용 안내: localStorage.hot_custom_tierX 키에 JSON 배열을 저장하면 UI에 즉시 반영됩니다.');
        
        // 모의 데이터 생성기 (할당량 절약용)
        this.mockDataGenerator = new MockDataGenerator();

        // 채널 커버리지 & 조회수 기준 추적
        this.coverageStats = this.createEmptyCoverageStats();
        this.currentViewFloor = 0;
        this.lastViewCountFilter = 'all';
    ...
    loadKeywordTier(storageKey, defaults = []) {
        try {
            const stored = localStorage.getItem(storageKey);
            if (!stored) return Array.from(new Set(defaults));
            const parsed = JSON.parse(stored);
            if (!Array.isArray(parsed)) return Array.from(new Set(defaults));
            const merged = [...defaults, ...parsed];
            return Array.from(new Set(merged.filter(Boolean).map(v => String(v).trim()))).filter(Boolean);
        } catch (error) {
            console.warn(`키워드 로드 실패 (${storageKey}):`, error);
            return Array.from(new Set(defaults));
        }
    }

    createEmptyCoverageStats() {
        return {
            targetChannels: 0,
    ...
    resetCoverageStats(targetChannels = 0) {
        this.coverageStats = this.createEmptyCoverageStats();
        this.coverageStats.targetChannels = Math.max(0, targetChannels || 0);
        this.updateCoverageDisplay();
    }

    bumpCoverageStats(partial = {}) {
        if (!this.coverageStats) {
            this.coverageStats = this.createEmptyCoverageStats();
        }
        const stats = this.coverageStats;
        stats.discoveredChannels += partial.discovered || 0;
        stats.queuedChannels += partial.queued || 0;
        stats.scannedChannels += partial.scanned || 0;
        stats.uploadsResolved += partial.uploads || 0;
        if (partial.target) {
            stats.targetChannels = Math.max(stats.targetChannels, partial.target);
        }
        this.updateCoverageDisplay();
    }

    updateCoverageDisplay() {
        const scannedEl = document.getElementById('scannedChannels');
        const coverageEl = document.getElementById('channelCoverage');
        if (!this.coverageStats) return;
        const { scannedChannels, targetChannels } = this.coverageStats;
        if (scannedEl) {
            scannedEl.textContent = `${scannedChannels.toLocaleString('ko-KR')} / ${targetChannels.toLocaleString('ko-KR')}`;
        }
        if (coverageEl) {
            const percent = targetChannels > 0 ? Math.min(100, Math.round((scannedChannels / targetChannels) * 100)) : 0;
            coverageEl.textContent = `${percent}%`;
        }
    }

    getViewCountFloor(filterValue = 'all') {
        if (!filterValue || filterValue === 'all') return 0;
        const numerical = Number(filterValue);
        if (Number.isFinite(numerical)) return numerical;
        const map = {
            '1000000': 1000000,
            '500000': 500000,
            '100000': 100000,
    ...
    // 최적화된 스캔 시작
        async startOptimizedScan() {
            ...
            try {
                ...
                this.lastViewCountFilter = viewCountFilter;
                this.currentViewFloor = this.getViewCountFloor(viewCountFilter);
            ...
            if (stats.remainingQuota <= 0) {
              ...
              if (typeof this.updateRealtimeDisplay === 'function') {
                this.updateRealtimeDisplay();  // #backgroundDataCount, #detectedVideos, #processingRate 반영
                    ...
                    this.resetCoverageStats(maxChannels);
    ...
    calculateComprehensiveViralScore(video) {
        const safeViews = Math.max(0, video.viewCount || 0);
        const daysOld = Math.max(1, video.daysSincePublish || 1);
        const engagementRatio = Math.max(0, (video.engagementRate || 0) / 100);
        const growthRatio = Math.max(0, video.growthRate || 0);
        const velocity = safeViews / daysOld;
        
        const components = {
            view: this.normalizeScore(Math.log10(safeViews + 1), 3, 7),             // 1천 ~ 1천만
            engagement: this.normalizeScore(engagementRatio, 0.01, 0.12),          // 1% ~ 12%
            velocity: this.normalizeScore(Math.log10(velocity + 1), 3, 7),
            growth: this.normalizeScore(Math.log10(growthRatio + 1), 1, 4),
            freshness: this.normalizeScore(Math.max(0, 30 - daysOld), 0, 30)
        };
        ...
        const absoluteFloor = Math.max(5000, this.currentViewFloor || 20000);
        if (safeViews < absoluteFloor) {
            const ratio = Math.max(0.1, safeViews / absoluteFloor);
            score *= ratio;
        }
        
        if (video.engagementRate < 1) {
            score *= 0.7;
        } else if (video.engagementRate < 2) {
            score *= 0.85;
        }
...
// 완료 메시지 업데이트
    ...
    async runChannelUploadPipeline(
      keywords,
      { format, timeRange, perChannelMax, topN, softTarget = 2000, dailyCapUnits = 8000, tier2Keywords = [], tier3Keywords = [], maxChannels: explicitMaxChannels, returnMeta = false } = {}
    ) {
      ...
      const effectiveMax = Number.isFinite(explicitMaxChannels) && explicitMaxChannels > 0
        ? explicitMaxChannels
        : maxChSetting;

        // ✅ 유니크 채널 기준으로 상한 적용
        const uniqueChannels = Array.from(new Set(channelsRaw));
        const channels = (effectiveMax === Infinity)
          ? uniqueChannels
          : uniqueChannels.slice(0, Math.max(1, Math.floor(effectiveMax)));
      ...
      if (!channels.length) {
        if (returnMeta) {
          return {
            videos: [],
            meta: {
              totalChannelsDiscovered: channelsRaw.length,
              uniqueCandidates: uniqueChannels.length,
              channelsQueued: 0,
              channelsScanned: 0,
              uploadsResolved: 0,
              videoCandidates: 0,
      ...
      if (returnMeta) {
        return {
          ...
          meta: {
            ...
            channelsScanned: valid.length,
            uploadsResolved: valid.length,
            videoCandidates: allIds.length,
            videosAfterStats: stats.length,
            videosReturned: top.length
          }
        };
      }
      return top;
...
try {
    ...
    async runSmartMode(category, format, count, limitedKeywords) {
      ...
      await this.processAndDisplayResults(count, this.lastViewCountFilter || 'all');
    }
    ...
    updateLargeScaleProgress(message, processedChannels, targetChannels, currentBatch, totalBatches) {
        const safeTarget = Math.max(1, targetChannels || 1);
        const channelPercent = Math.round((processedChannels / safeTarget) * 100);
        const safeBatchTotal = Math.max(1, totalBatches || 1);
        const batchPercent = Math.round((currentBatch / safeBatchTotal) * 100);
    ...
    applyViewCountFilter(videos, viewCountFilter) {
        ...
        const minViewCount = this.getViewCountFloor(viewCountFilter);
        if (!minViewCount) {
            return videos;
        }
    ...
    updateLiveCountersUI() {
      ...
      this.updateCoverageDisplay();
        ...
        async runLargeScaleChannelPipeline(keywords, options = {}) {
            ...
            const targetChannelCount = Math.max(1, maxChannels || batchSettings?.batchSize || 1);
            const channelLimit = maxChannels || targetChannelCount;
            
            console.log(`🔄 대용량 파이프라인 시작: ${targetChannelCount.toLocaleString('ko-KR')}개 채널 목표`);
            ...
            let currentBatch = 0;
            if (channelLimit && !this.coverageStats?.targetChannels) {
                this.resetCoverageStats(channelLimit);
            }
            
            try {
                // 키워드별로 배치 처리 - 필터링 없이 모든 영상 수집
                for (const keyword of keywords) {
                    if (processedChannels >= channelLimit) break;
                    
                    const remainingChannels = channelLimit - processedChannels;
                    ...
                    const pipelineResult = await this.runChannelUploadPipeline(
                        [keyword],
                        {
                            ...options,
                            ...
                            maxChannels: currentBatchSize,
                            returnMeta: true
                        }
                    );
                    
                    const batchVideos = Array.isArray(pipelineResult) ? pipelineResult : (pipelineResult?.videos || []);
                    const coverageMeta = Array.isArray(pipelineResult) ? null : (pipelineResult?.meta || null);
                    
                    if (batchVideos.length > 0) {
                        allRawResults.push(...batchVideos);
                        const newlyScanned = coverageMeta?.channelsScanned ?? coverageMeta?.channelsQueued ?? batchVideos.length;
                        processedChannels += newlyScanned;
                        
                        // 실시간 카운터 업데이트
                        this.realTimeCounters.backgroundData += batchVideos.length;
                        this.updateRealtimeDisplay();
                        
                        this.bumpCoverageStats({
                            discovered: coverageMeta?.totalChannelsDiscovered || 0,
                            queued: coverageMeta?.channelsQueued || 0,
                            scanned: newlyScanned || 0,
                            uploads: coverageMeta?.uploadsResolved || 0,
                            target: channelLimit
                        });
                    }
                    
                    // 배치 간 딜레이 (API 안정성)
                    if (currentBatch < keywords.length && processedChannels < channelLimit) {
                        await this.delay(batchSettings.delayMs);
                    }
                    
                    console.log(`✅ 배치 ${currentBatch} 완료: ${batchVideos.length || 0}개 결과 (누적: ${allRawResults.length.toLocaleString('ko-KR')}개)`);
                }
